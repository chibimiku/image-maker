...
分析产物(JSON) ──► 首图  /v1/images/generations（画风 prompt_gpt + 分析描述 + 排除句 + 渲染语言条款）
                 ...
                 重绘  gemini-3-pro-image-preview，2K（源图 + 画风图 双参考，v5 固件 + STYLE LANGUAGE
                       + 「重绘范围」句 + 身份/内容锁）—— **一整张新图，不裁切不贴回**
                    ▼
                 结构线 sline50（纯本地：抽长结构边 → 按局部色调整色 → 叠回）
                    ▼
                 色调校准（目标 = 画风参考图，本地）+ 线条加墨（本地）
                    ▼
                 终图  data/<日期>/…-final-rp+sline50+tone+ink.png
                       （只有显式勾了局部重绘才会出现 +local-… 段；那条路已知会拼坏，§7.7）
```
...
| 局部重绘 | **默认关闭**（裁切→重绘→贴回会把画面拼坏，§7.7；要开只能显式勾，且优先用下面的「重绘范围」） | — |
| **重绘范围** | `repaint.scope`，5 档：`lines_only`（**默认**）/ `person_only` / `person_noface` / `details` / `full` | **不裁切不贴回**，只在提示词里要求模型保持不该动的部分；`lines_only` 实测线条最好（tid 段长 74.7→185.0） |
...
**别做的事**：① 不要用线锚图（`reference_mode=line_anchor/both`）当重绘参考 —— 会把画面塌成白底线稿或铺满「碎玻璃」纹理；② **不要再开默认的四区局部重绘**：裁切→重绘→贴回会被模型"重新构图"变成错位块（§7.7，5 画风报废 2 张）；③ 不要把 4K 当默认（局部统一 2K）；④ 改完 `.py` 一定要**重启 app**，否则跑的还是旧提示词。
...
**改法（用户已拍板，见 §7.9）**：**不做硬拼接**，改成「整张新图 + 提示词里的编辑范围要求」，
已实现并设成默认；裁切式局部重绘降级为默认关闭的可选项。

> 结论：**「repaint → sline50 → 四区 → tone + ink」里的 `四区` 不能当默认配方用**；
> `repaint / sline50 / tone / ink` 四步都成立。GUI 默认里的四区链在用户确认前不要当成已验证配置。

### 7.9 ✅ 已落地的替代方案：用「重绘范围」代替硬拼接（2026-09-24，`recipe_version=3`）

用户指定：**不做裁切/贴回，图始终是一整张新图**，靠重绘提示词里的不同要求让模型保持不该动的部分。
实测（3 个基底 × 5 档，每次带画风参考图 + 条款，完整表见 `repaint-scope.html` / `BEST-PIPELINE.md` §三十一）：

| 档位 | 语义 | 实测（段长，基底=各画风首图） |
|---|---|---|
| `full` | 不额外限制 | tinkle 134.8 / tid 105.1 / fuzichoco 81.5 |
| `person_only` | 只重绘人物，背景原样 | 87.0 / 111.6 / 81.2 |
| `person_noface` | 人物可动、脸原样 | 100.9 / 139.9 / 82.2 |
| `details` | 只修手/丝带/系带/袜带/鞋带/项链 | 103.5 / 99.6 / 82.5 |
| **`lines_only`（默认）** | 只连通线条，不改色不改内容 | 98.5 / **185.0** / 83.3 |

- 关键收益：**几何天然对齐，不可能再出现拼坏块**；`lines_only` 在 tid 上把段长从 74.7 拉到 185.0
  （超过 tid 自己参考图的 92.4），碎线 0.017 → 0.003。
- `details` / `person_only` 的背景保持最好；`person_noface` 改动最多。
- 代码：`post_process.REPAINT_SCOPE_CLAUSES` / `REPAINT_SCOPE_LABELS`、
  `pipeline_steps_from_flags(repaint_scope=...)`、GUI「重绘范围」下拉、CLI `--repaint-scope`；
  `local`（裁切贴回）默认关闭，超时预算变成「首图 + 重绘」= 2 份。
- **还没做**：用新配方把 5 个画风从头复跑做横向验收（下一个 agent 可以拿这个当第一件事）。

### 7.9.1 根因复核：角色被参考图替换是**低概率采样事故**，不是"范围句必然导致"（2026-09-24 深夜）

用户追问「身体元素为什么变成参考图的」。复核（`log/leak-check.txt`、`log/leak-repro-nolock.txt`）：

- **量化泄漏检测**（头发区/躯干区里"参考图特征色"占比，判据：躯干藏青 >5% 或 头发粉 >15%）：
  加锁前那张 = 头发粉 **23.6%** / 躯干藏青 **25.4%**（首图是 6.2% / 0.0%）→ 确实整套角色被搬；
  加锁后 8 次 = 头发粉 6.9~10.8% / 藏青 0.0~0.1% → 全部正常。
- **把「加锁前」的原提示词原样重跑 3 次 → 0/3 复现** → 所以它**不是确定性的**，
  当前样本下发生率 < ~15%。我上一轮写"原因就是范围句允许改身体/衣服/头发"**说得太满**。

**机制（两条叠加）**：① 画风参考图是一张**完整角色插画**；② `person_noface` 明确允许改头发/衣服/身体 ——
模型偶尔会把"参考图"当"设计来源"，把它的角色（粉发 + 水手服 + 金鱼/水波）整套搬进来。
身份锁把发生率压下去了，但要**根治**得从结构上去掉角色插画：

1. 允许改设计的档位（`person_noface` / `person_only` / `full`）**不挂参考图**，风格靠 STYLE LANGUAGE 条款 + 色调校准；
2. 或把参考图预先做成**无角色的风格板**（裁掉/擦掉主体）再挂；
3. 或只用渲染类档位（`lines_only` / `details`）—— 它们不要求改头发/衣服，天然没有这个风险。

（三条都还没做。证据页：`repaint-scope.html` 索引 + 泄漏检测表 → `repaint-scope-after-lock.html` 复测。）
